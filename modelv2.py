...
def load_video(video_path, max_frames_num, fps=25, force_sample=False):
    if max_frames_num == 0:
...
model.eval()
video_path = "30fps1920/output/final/annotated.mp4"
max_frames_num = 300
...
time_instruciton = f"The video lasts for {video_time:.2f} seconds, and {len(video[0])} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following questions related to this video.\nAdditionally, here is all the data extracted from the video: {frames_data}"
question = (
    DEFAULT_IMAGE_TOKEN
    + f"\n{time_instruciton}\nAs a squash coach, describe this video in extreme detail"
)
