...
def analyze_frame(row, prev_positions):
    ...
    p1_current_pos = (
        np.array([p1_x_avg, p1_y_avg])
        if p1_x_avg is not None and p1_y_avg is not None
        else None
    )
    ...
    # Create a numpy array for current position
    p2_current_pos = (
        np.array([p2_x_avg, p2_y_avg])
        if p2_x_avg is not None and p2_y_avg is not None
        else None
    )
...
def main():
    prev_positions = {"Player 1 Numerical": None, "Player 2 Numerical": None}
        ...
        try:
            ...
            if points[0][0] != 0 and points[1][0] != 0:
                ...
                if abs(head_tilt) > 0.05:
                    description.append(
                        f"head tilted {'right' if head_tilt > 0 else 'left'}"
                    )
                else:
    ...
    try:
        with open(filename, "r") as file:
            ...
            for row in reader:
                ...
                description += (
                    f"Player 2 is {p2_description} and is {p2_ball_pos} the ball. "
                )
