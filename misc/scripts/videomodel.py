...
class VideoTextModel:
    def __init__(self, video_path, prompt):
        ...
        self.processor = VideoTextToTextProcessor.from_pretrained(
            "huggingface/video-text-to-text"
        )
        self.model = VideoTextToTextModel.from_pretrained(
            "huggingface/video-text-to-text"
        )
...
if __name__ == "__main__":
    ...
    if __name__ == "__main__":
        video_path = "annotated.mp4"
        prompt = (
            "You are a squash coach, coach me as I am the guy in the black shirt(id 2)"
        )
        model = VideoTextModel(video_path, prompt)
        print(model.generate_text())
